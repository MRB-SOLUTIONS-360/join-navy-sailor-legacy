...
use common\static\SendSms;
use Exception;
use yii\base\DynamicModel;

class SailorCandidateController extends \yii\web\Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['verify-candidate'],
                        'roles' => ['?'],
                    ],
                    [
                        'allow' => true,
                        'actions' => ['payment', 'academic-info'],
                        'roles' => ['@'],
                    ],
                ],
            ],
        ...
        if ($sailor) {
            ...
            if ($sailor->load(Yii::$app->request->post())) {
                if ($sailor->validate()) {
                    ...
                    // if ($sailor->payment_status == Constants::PAYMENT_UNPAID && !empty($sailor->validation_id)  && $payment_mode == Constants::PAYMENT_MODE_LIVE) {
                    if ($sailor->payment_status == Constants::PAYMENT_UNPAID && !empty($sailor->validation_id)) {
                        ...
                        $responseData =  SSLPayment::allRequestListByTranIds(app_type: 'sailor', payment_mode: $payment_mode, tranIds: $tran_ids);                       
                    ...
                    // generate every request for payment 
                    //$sailor->order_id_original = date('ymd') . rand(10, 99) . $sailor->id;
                    $sailor->validation_id = date('Ymdhis') . rand(111, 999) . $sailor->id;
                    /// Payment info                  
                    $payment_info = [
            ...
            $sailor->refund_phone =  ($sailor->refund_phone) ? DataEncryption::dataDecrypt($sailor['refund_phone']) : '';
    ...
    public function actionAcademicInfo($slug = null)
    {
        ...
        if ($sailor) {
            ...
            // set ssc academic group from eligibility form 
            $eligibility_info = CanEligibilityCheckInfo::find()->select(['id', 'ssc_equv_group', 'hsc_equv_group'])->where(['id' => $sailor->eligibility_info_id])->one();
            $sailor->ssc_group = $eligibility_info->ssc_equv_group;
            if ($eligibility_info->hsc_equv_group)
                $sailor->hsc_dip_group = $eligibility_info->hsc_equv_group;
            ...
            if ($sailor->candidate_designation == Constants::TOPASS_PRIMARY_KEY)  // only toppas   candidate_designation should be apply_department
                $sailor->scenario = $sailor::ACADEMIC_INFO_JSC;
            else
                $sailor->scenario = $sailor::ACADEMIC_INFO_JSC_SSC;
                    ...
                    if ($sailor->validate()) {
                        ...
                        if ($sailor->ssc_edu_board && $sailor->ssc_reg_no && $sailor->ssc_roll_no && $sailor->ssc_passing_year) {
                            ...
                            if (!empty($ssc_result) && $ssc_result->responseDesc == 'Success' && ($ssc_result->result == 'P' || $ssc_result->result == 'PA' || $ssc_result->result == 'PB' || $ssc_result->result == 'PC')) {
                                ...
                                } else if ($eligible_config && $sailor->ssc_edu_board == Constants::EDU_BOARD_TEC && $sailor->candidate_designation == 4) {  // 4 = Medical                            
                                    if ($resultArr['gpa'] < $eligible_config['ssc_result']) {
                                        $sailor->addError('academic_info_already_used_in_ssc', ' আপনার প্রাপ্ত জিপিএ  ' . $resultArr['gpa'] . ' যা ' . $eligible_config['ssc_result'] . ' অপেক্ষা কম।');
                                        return $this->render('academic_info',  ['model' => $sailor]);
                                    } else if (!in_array(strtolower($resultArr['studGroup']), explode(',', $eligible_config['ssc_ac_group']))) {
                                ...
                                return $this->redirect(Yii::$app->urlManager->createUrl(["sailor-candidate/academic-info", 'slug' => $slug]));
                            }
                        }
                        /// HSC Academic Result 
                        if ($sailor->hsc_dip_board && $sailor->hsc_dip_reg_no && $sailor->hsc_dip_roll_no && $sailor->hsc_dip_passing_year) {
                            ...
                            if (!empty($hsc_result) && $hsc_result->responseDesc == 'Success' && ($hsc_result->result == 'P' || $hsc_result->result == 'PA' || $hsc_result->result == 'PB')) {
                                ...
                                // if api information and post information not same 
                                //  if( $hsc_result->rollNo!= $sailor->hsc_dip_roll_no || $hsc_result->passYear!= $sailor->hsc_dip_passing_year ||  $hsc_result->regNo!= $sailor->hsc_dip_reg_no  || strtolower($hsc_result->board)  != strtolower ($sailor->hsc_dip_board) )
                                //  {
                                //      Yii::$app->session->setFlash('error', "Please provide your valid HSC academic informaton");
                                //      return $this->render( 'academic_info',  [ 'model' => $sailor ]);
                                //  }
                        ...
                        if ($sailor->save())
                            return $this->redirect(Yii::$app->urlManager->createUrl(["sailor-candidate/personal-info", 'slug' => $slug]));
                        else {
                            echo '<pre>';
                            print_r($sailor->getErrors());
                            echo '</pre>';
        ...
        if ($sailor) {
            ...
            if ($sailor->load(Yii::$app->request->post())) {
                ...
                $batch_model = SailorBatchs::find()->select(['id', 'name_en'])
                    ->where(['id' => $sailor->batch_id])->asArray()
                    ->one();
                ...
                if (!is_dir(Yii::getAlias('@rootDirFilUpload') . '/media/sailor_candidate/' . $sailor->id)) {
                    mkdir(Yii::getAlias('@rootDirFilUpload') . '/media/sailor_candidate/' . $sailor->id, 0777);
                }
                /// check application is close or not
                // if close , candidate can not apply 
                $can_apply = SailorBatchs::isCandidateContinueApplication(batch_id: $sailor['batch_id'], isPaid: $sailor['payment_status']);
                ...
                // $rq_name = '/media/sailor_candidate/' . $sailor->id . '/qr.png';
                // if (empty($sailor->qr_photo) || !file_exists(Yii::getAlias('@rootDirFilUpload') . $rq_name)) {
                //     $verification_link = Yii::getAlias('@rootMediaShow') . '/sailor-candidate/verify-candidate/?slug=' . $slug;
                ...
                if ($fileImage) {
                    try {
                        ...
                        $sailor->photo = '/media/sailor_candidate/' . $sailor->id . '/' . $uniqueName . '.' . $fileImage->extension;
                        $path  = $uploadBasePath . $sailor->photo;
                        $unlink_dir = $uploadBasePath .'/media/sailor_candidate/' . $sailor->id;
                ...
                if ($sailor->validate()) {
                    ...
                    return $this->redirect(Yii::$app->urlManager->createUrl(["sailor-candidate/application-preview", 'slug' => $slug]));
                } else {
    ...
    public function actionApplicationPreview($slug = null)
    {
        ...
        if ($sailor) {
            ...
            // if ($sailor->current_union)
            //     $sailor->current_union =  Unions::unionNameById($sailor->current_union);
            // if ($sailor->permanent_union)
            //     $sailor->permanent_union =   Unions::unionNameById($sailor->permanent_union);
            if ($sailor->permanent_thana)
            ...
            if (Yii::$app->request->isPost) {
                ...
                if ($batch_setting_info['roll_from'] == Constants::ROLL_FROM_BATCH) {
                    ...
                    if (empty($configuration_model)) {
                        Yii::$app->session->setFlash('application_close', "Configuration missing please contact with support.1");
                        return $this->redirect(Yii::$app->urlManager->createUrl(["/my-application"]));
                    } else {
                        /// check have candidate this batch with roll 
                        /// Roll No  generate by Batch                                 
                        $roll_no_details = Sailors::nextRollByBatchId(
                            batchId: $sailor['batch_id'],
                            batch_setting_info_roll_no: $batch_setting_info['start_roll'],                        
                        );
                        $roll_no  =  $roll_no_details['roll_no'];                 
                        if ($batch_setting_info['next_start_roll'] &&  $batch_setting_info['next_start_roll_after'] &&  $roll_no == ($batch_setting_info['next_start_roll_after'] + 1)) {
                            $roll_no = $batch_setting_info['next_start_roll'];
                        }
                        ...
                        if (count($configuration_model) > 1) {
                            ...
                            $sailor_batch_wise_count  =  Sailors::sailorBatchAndGroupWiseCount(batch_id: $sailor['batch_id'], center_id: $sailor['center_id'], gender: $sailor['gender'], candidate_designation: $sailor['candidate_designation'], eligible_district: $sailor['eligible_district'], batch_config_keys: $batch_config_keys);
                        ...
                        $exam_date_details = Sailors::getLastRollExamDateByDesignationCenter(
                            batchId: $sailor['batch_id'],
                            canInfo: $canInfo,                        
                        );                           
                        ...
                        if($configuration_model['check_max_candidate'] == Constants::YES   ){
                            ...
                            {
                                $count = Sailors::getTotalCandidateByExamDate(
                                    batch_id:$sailor['batch_id'],
                        ...
                        $sailor->team = $configuration_model['team'];
                        ...
                        if ($sailor->save(false)) {
                            ...
                            //  $smsBody = 'Bangladesh NAVY - ' . $sailor->name . ' Your Roll Number Changed. Download from this link '.'https://joinnavy.navy.mil.bd/sailor/downloadslip/id/'. $r_id;  
                            // $smsBody = 'Bangladesh NAVY - ' . $sailor->name . ' Your Roll Number Changed. Download from this link ';
                            // SendSms::sendSms(mobile: $sailor->permanent_phone, smsBody: $smsBody, serial_no: $sailor->serial_no,  application_type: 'sailor');
                            // generate log after complete application 
                            Sailors::generateLog($sailor->id);

                            return $this->redirect(Yii::$app->urlManager->createUrl(["sailor-candidate/download", 'slug' => $slug]));
                        } else {
    ...
    public function actionDownload($slug = null)
    {
        ...
        if ($sailor) {
            ...
            // $mpdf->debug = false;
            $mpdf->WriteHTML($this->renderPartial('candidate/application_form_pdf', ['model' => $sailor], true, false));
    ...
    public function actionDownloadForm($slug = null)
    {
        ...
        if ($sailor) {
            ...
            // add +88 to phone number
            $sailor->current_phone = '+88' . $sailor->current_phone;
            $sailor->permanent_phone = '+88' . $sailor->permanent_phone;            
    ...
    public function actionRefundPhone()
    {
        $userId = Yii::$app->user->id;

        if (Yii::$app->request->isAjax && $userId) {
            $post = Yii::$app->request->post()['Sailors'];

            $error = [];
            if (empty($post['refund_phone']))
                $error['refund_phone'] = 'রিফান্ড ফোন নাম্বার খালি রাখা যাবে না।';
            if (empty($post['id']))
                $error['id'] = 'id is required';

            if ($post['refund_phone'] && !empty($post['refund_phone']) && !preg_match('/^(?:\+88|88)?(01[3-9]\d{8})$/', $post['refund_phone'])) {
                $error['refund_phone'] = 'ফোন নম্বর সঠিক নয়।বাংলাদেশী বৈধ মোবাইল নাম্বার ব্যাবহার করুন।';
            }
            ...
            $sailorModel =  Sailors::find()->select(['id', 'refund_phone', 'created_by'])
                ->where(['id' => $post['id']])
                ->andWhere(['created_by' => $userId])
                ->one();

            if (empty($sailorModel)) {
                return \yii\helpers\Json::encode([
                    'success' => false,
                    'errors' => ['refund_phone' => 'Something went wrong.'],
                ]);
            }

            $sailorModel->refund_phone =  DataEncryption::dataEncrypt($post['refund_phone']);

            if ($sailorModel->save(false)) {
                return \yii\helpers\Json::encode([
                    'success' => true,
    ...
    public function actionCancelApplication()
    {
        ...
        if (Yii::$app->request->isAjax && $userId) {
            ...
            $sailorModel =  Sailors::find()->select(['id', 'request_for_cancel', 'created_by','reason', 'app_unique_id'])
                ->where(['id' => StaticMethod::decryptPk($post['slug'])])
                ->andWhere(['created_by' => $userId])
                ->one();
            ...
            if (empty($sailorModel)) {
                return \yii\helpers\Json::encode([
                    'success' => false,
                    'errors' => ['slug' => 'Something went wrong.'],
                ]);
            }

            $sailorModel->request_for_cancel = 1;
            $sailorModel->reason = $post['reason'];

            if ($sailorModel->save(false)) {
                return \yii\helpers\Json::encode([
                    'success' => true,
                    'message' => 'Your request accepted successfully. Wait for confirmation.'
                ]);
            } else {
                return \yii\helpers\Json::encode([
                    'success' => false,
